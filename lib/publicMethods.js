...
export default function (self) {
      ...
      if (
        ...
        if (
          ...
          (self.attributes.allowColumnResize || moveBorder) &&
                    !(
            self.attributes.fillLastColumn &&
            border === 'r' &&
            cell.sortColumnIndex === self.getLastVisibleColumnIndex()
          ) &&
  ...
  self.getColumnWidth = function (columnIndex) {
    ...
    if (
      ...
      self.autoLastColumnWidth.index === columnIndex
    ) {
  ...
  self.getLastVisibleColumnIndex = function () {
    ...
    const isColumnCollapsed = (columnIndex) =>
      collapsedColumnGroups.some(
        (group) => columnIndex >= group.from && columnIndex <= group.to,
      );
    for (
      let viewIndex = self.orders.columns.length - 1;
      viewIndex >= 0;
      viewIndex -= 1
    ) {
      ...
      if (
        schema[schemaIndex] &&
        !schema[schemaIndex].hidden &&
        !isColumnCollapsed(viewIndex)
      ) {
        return schemaIndex;
      }
    }
